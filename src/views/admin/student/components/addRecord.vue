...
export default {
  name: "add",
  props: ['stuId', 'stuName', 'stuDorm', 'stuClass', 'addType', 'dormHeader', 'monitor'],
  data() {
    return {
      addForm: {
        civilType: '',
  ...
  methods: {
    ...
    open1() {
      this.$notify({
        title: this.addForm.dorm + this.addForm.name + '加分成功',
    ...
    open3() {
      this.$notify({
        title: this.addForm.dorm + this.addForm.name + '扣分成功',
        dangerouslyUseHTMLString: true,
    ...
    dataCommit: function () {
      ...
      if (this.addForm.dorm !== '') {
        queryData = queryData + '&stuDorm=' + this.addForm.dorm
      }
      if (this.addForm.class !== '') {
        queryData = queryData + '&stuClass=' + this.addForm.class
      }
      ...
      qcxUtils.ajaxHandle(this, settings, res => {
        if (res !== undefined) {
          ...
          if (this.addForm.civilScore >= 0 && res.status == 200) {
            this.open1()
          }
          else if (this.addForm.civilScore < 0 && res.status == 200) {
            this.open3()
          }
          else {
            this.open2()
